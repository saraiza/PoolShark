...
MyClass co;
co.m_companyName = QString("908 Devices");
co.m_iEmployees = 9;
...
Here at 908 Devices, I felt it was time to give it a nice botox
injection.  It is now natively based on Qt and I added dynamic 
object serialization (Qt-style which means that you can serialize 
pointers to classes. It also handles duplicate objects correctly, you 
can now serialize a complex tree of objects.
...
Because of this, the actual object version number is no longer sequential.  We use 
a randomized and hopefully unique number (at 908 Devices, this number is randomly 
generated by a central webserver on request by a developer).
